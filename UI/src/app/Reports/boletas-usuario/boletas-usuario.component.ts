...
@Component({
  selector: 'app-boletas-usuario',
  templateUrl: './boletas-usuario.component.html',
  styleUrls: ['./boletas-usuario.component.css']
})
export class BoletasUsuarioComponent implements OnInit {
...
@ViewChild('htmlData') htmlData:ElementRef;
  ...
  constructor(private BoletaService:BoletaService,
    ...
    private config: NgSelectConfig,private UsuarioService:UsuarioService) { 
    this.spinner.show();
    this.getUsuarios();
     this.getBoletas();
  }
  ...
  filterRPT(){
    if(this.selected!=0){
      this.spinner.show();
      this.ReportsService.getReportForUser(this.selected).subscribe(
        res=>{
          this.boletas=res;
          this.spinner.hide();
          this.openPDF();
