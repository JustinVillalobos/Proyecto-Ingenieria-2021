...
@Component({
  selector: 'app-boletas-departamento-mes',
  templateUrl: './boletas-departamento-mes.component.html',
  styleUrls: ['./boletas-departamento-mes.component.css']
})
export class BoletasDepartamentoMesComponent implements OnInit {
  ...
  filterRPT(){
    if(this.selected!=0 && this.selectedMes!=0){
      this.spinner.show();
      this.ReportsService.getReportForDepartmentAndDate(this.selectedMes,this.selected).subscribe(
        res=>{
          this.boletas=res;
          this.spinner.hide();
          this.openPDF();
